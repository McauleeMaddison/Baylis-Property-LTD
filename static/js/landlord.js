(() => {
  ...
  const TOUR_KEY = "tour:landlord:v2";
  ...
  const els = {
    ...
    rollingUpdates: document.getElementById("landlordRollingUpdates"),
  ...
  async function ensureCsrf() {
    ...
    try {
      await fetch(`${API_BASE}/security/csrf`, {
        credentials: "include",
      });
    } catch (_) {}
  }
  ...
  async function loadDashboard() {
    try {
      ...
      state.notifications = Array.isArray(notifications?.notifications)
        ...
        : [];
      state.properties = Array.isArray(propertyData?.properties)
      ...
      showError(err.message || "Unable to load landlord data.");
    }
  }

  function showError(message) {
    [els.requests, els.posts, els.propertySummary, els.notifications].forEach((list) => {
      if (!list) return;
  ...
  function renderPropertyFilter() {
    ...
    const options = ['<option value="all">All properties</option>'].concat(
    ...
    const hasPrevious = Array.from(els.filterProperty.options).some((opt) => {
  ...
  function getFilteredRequests() {
    const search = String(els.search?.value || "").toLowerCase().trim();
    const type = String(els.filterType?.value || "all").toLowerCase();
    const status = String(els.filterStatus?.value || "all").toLowerCase();
    const property = String(els.filterProperty?.value || "all").toLowerCase();
    ...
    return state.requests.filter((req) => {
      ...
      const matchesType =
        type === "all" ||
        String(req.type || "").toLowerCase() === type;
  ...
  function renderRequests() {
    ...
    const filtered = getFilteredRequests().slice().sort((a, b) => {
      const byProperty = String(a.propertyLabel || "").localeCompare(
  ...
  function renderRequestItem(req) {
    ...
    const typeClass = req.type === "cleaning"
      ? "request-entry--cleaning"
      : "request-entry--repair";

    return `
      <div class="request-entry request-entry--landlord ${typeClass}">
        <div class="section-row">
          <strong>${escapeHtml(req.name || req.user || "Resident")}</strong>
          <span class="chip ${req.type === "cleaning" ? "cleaning" : "repair"}">
            ${escapeHtml(req.type || "request")}
          </span>
        </div>
        ...
        <div class="request-entry-detail">${escapeHtml(formatSla(req))}</div>

        ${photos}

        <div class="request-actions">
          <span class="badge ${badgeClass}">
            Status: ${escapeHtml(status.replace("_", " "))}
          </span>
  ...
  function renderPropertySummary() {
    ...
    state.properties.forEach((property) => {
      const stats = countsById.get(property.id) || {
        open: 0,
        inProgress: 0,
        done: 0,
        total: 0,
    ...
    cards.forEach((card) => {
      ...
      const selected = String(els.filterProperty?.value || "all") === card.id;
      ...
      li.innerHTML = `
        <div>
          ...
          <div class="muted">
            ...
            In progress ${card.stats.inProgress} •
          ...
          <div class="property-summary-actions">
            <button
              ...
              data-action="filter-property"
              data-property-id="${card.id}"
            >
              ${selected ? "Clear filter" : "View requests"}
            </button>
  ...
  function renderCommunityItem(post) {
    ...
    return `
      <div>
        ...
        <div class="muted">${escapeHtml(post.author || "Anon")} • ${escapeHtml(when)}</div>
  ...
  function getRequestSummary(requests = getFilteredRequests()) {
  ...
  function renderRollingUpdates() {
    ...
    const summary = getRequestSummary(filtered);
    const unread = state.notifications.filter((notif) => !notif.readAt).length;

    const latestRequest = state.requests.slice().sort((a, b) => {
      return new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime();
    })[0];
    ...
    const updateCards = [
      ...
      },
      {
        ...
        detail: "Active resident work orders",
      },
      {
        label: "Completed",
        value: summary.done,
        detail: "Resolved resident requests",
      },
      {
        label: "Overdue",
        value: summary.overdue,
        detail: "Past expected SLA",
      },
      {
        label: "Properties",
        value: state.properties.length,
        detail: "Managed property records",
      },
      {
        label: "Notifications",
        value: unread,
      ...
      },
      {
        ...
        detail: latestAudit?.event || "No recent audit events",
      },
    ];

    const requestCards = latestRequest
      ? [
          {
            label: "Latest Request",
            value: latestRequest.type === "cleaning" ? "Cleaning" : "Repair",
            detail: latestRequest.type === "cleaning"
              ? latestRequest.cleaningType || "Cleaning request"
              : latestRequest.issue || "Repair request",
          },
          {
            label: "Latest Property",
            value: latestRequest.propertyLabel || "Unmapped",
            detail: latestRequest.name || latestRequest.user || "Resident request",
          },
        ]
      : [
          {
            label: "Latest Request",
            value: "None yet",
            detail: "New resident requests will appear here",
          },
        ];
    ...
    const notificationCards = state.notifications.slice(0, 4).map((notif) => ({
      ...
      detail: notif.body || "You have an operational update",
    }));

    const allCards = updateCards.concat(requestCards, notificationCards);
    const rollingCards = allCards.concat(allCards);
  ...
  function renderNotifications() {
    ...
    if (!state.notifications.length) {
      ...
      if (els.notifMarkAll) {
        els.notifMarkAll.disabled = true;
      }
    ...
    if (els.notifMarkAll) {
      els.notifMarkAll.disabled = false;
    }
    ...
    state.notifications.forEach((notif) => {
      ...
      li.innerHTML = `
        ...
        <div class="notification-meta">
          ${escapeHtml(fmtDateTime(notif.createdAt))}${isRead ? " • Read" : ""}
        </div>
      `;
  ...
  function renderAudit() {
    ...
    if (!state.auditLogs.length) {
      els.auditBody.innerHTML = `
        <tr>
          <td colspan="4" class="muted">No audit events yet.</td>
        </tr>
      `;
    ...
    state.auditLogs.forEach((log) => {
      ...
      tr.innerHTML = `
        ...
        <td data-label="Severity">
          <span class="badge">${escapeHtml((log.severity || "info").toUpperCase())}</span>
        </td>
  ...
  function formatAuditDetails(log) {
    ...
    Object.keys(meta).forEach((key) => {
      ...
      const val = typeof meta[key] === "object"
  ...
  function hydrateList(host, items, renderFn, emptyHtml) {
    ...
    if (!items.length) {
      host.innerHTML = typeof emptyHtml === "function"
        ? emptyHtml()
        : `<li class="muted">${escapeHtml(String(emptyHtml || ""))}</li>`;
  ...
  function renderPhotoStrip(photos) {
    ...
    const thumbs = photos.slice(0, 3).map((src) => {
      return `<img src="${encodeURI(src)}" alt="Repair photo" loading="lazy" />`;
    }).join("");
  ...
  function emptyStateHtml({ title, body, actions = [] }) {
    const buttons = actions.map((btn) => `
      <button type="button" class="btn btn-ghost btn-small" data-action="${btn.action}">
  ...
  function bindStatusEvents() {
    els.requests?.addEventListener("change", async (event) => {
      ...
      try {
        const updated = await fetchJSON(`/requests/${id}/status`, {
          method: "POST",
          body: { status },
        });

        const idx = state.requests.findIndex((requestItem) => {
          return String(requestItem.id) === String(id);
        });

        if (idx >= 0 && updated) {
          state.requests[idx] = normalizeRequest(updated);
        }
  ...
  function bindPropertySummaryActions() {
    els.propertySummary?.addEventListener("click", async (event) => {
      ...
      if (action === "filter-property") {
        ...
        els.filterProperty.value = els.filterProperty.value === propertyId
          ? "all"
          : propertyId;
  ...
  async function editProperty(propertyId) {
    ...
    if (normalized === String(property.label || "").trim()) {
      return;
    }

    try {
      await fetchJSON(`/properties/${encodeURIComponent(propertyId)}`, {
        method: "PATCH",
        body: { label: normalized },
      });
  ...
  function maybeStartTour() {
    ...
    const steps = [
      ...
      },
      {
        ...
        body: "This rolling section shows live request totals, notifications, properties, and audit activity.",
        target: ".rolling-updates--landlord",
      },
      {
